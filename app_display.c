...
{
    const nrf_gfx_font_desc_t *m_font = &arialRoundedMTBold_6ptFontInfo;
    ...
    uint32_t err_code = nrf_gfx_print(&m_led_matrix, &point, color, text, &arialRoundedMTBold_6ptFontInfo, false);    
}
...
{
    app_display_draw_int(gamestate->player[0].score, 16, 22, gamestate->player[0].color, ALIGNMENT_CENTRE);
    app_display_draw_int(gamestate->player[1].score, 48, 22, gamestate->player[1].color, ALIGNMENT_CENTRE);
}
