...
void app_display_draw_paddles(uint32_t p1_pos, uint32_t p2_pos)
{
    ...
    drv_led_matrix_draw_rectangle(0, p1_pos - (PADDLE_HEIGHT / 2), 2, PADDLE_HEIGHT, CL_GREEN);

    
    drv_led_matrix_draw_rectangle(62, 0, 2, 32, CL_BLACK);
    drv_led_matrix_draw_rectangle(62, p2_pos - (PADDLE_HEIGHT / 2), 2, PADDLE_HEIGHT, CL_GREEN);
}
...
void app_display_draw_ball(uint32_t pos_x, uint32_t pos_y)
{
    ...
    drv_led_matrix_draw_rectangle(pos_x + 2, pos_y, 2, 2, CL_PURPLE);    
